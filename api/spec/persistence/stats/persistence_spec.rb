...
RSpec.describe Stats::Persistence, truncate_click_house: true do
  ...
  shared_examples 'able to use stats' do
    describe '#create' do
      let(:expected_ping) do
        { ip: IPAddr.new(ip), ping: time.round(3), pong: (time + 0.1).round(3), duration: 0.1, timeout: false }
      end
      let(:ping) { pool.select_one("SELECT * FROM #{table} ORDER BY ping DESC LIMIT 1") }

      it 'sets correct ping time' do
        subject.create(ip, ping: time, pong: time + 0.1)
        expect(ping[:ping] - expected_ping[:ping]).to be <= 0.001
      end

      it 'sets correct pong time' do
        subject.create(ip, ping: time, pong: time + 0.1)
    ...
    describe '#get' do
      %i[count avg max min stddev median timeout_percent].each do |metric|
        it "returns correct #{metric} if pings exist" do
          subject.create(ip, ping: time, pong: time + 0.1)
          ...
          expected_stats = { count: 4, avg:, max: 0.8, min: 0.1, stddev: expected_deviation, median: 0.6,
                             timeout_percent: 0.25 }
          expect(described_class.new.get(ip)[metric]).to be_within(0.0005).of(expected_stats[metric])
        end
      end

      it 'returns 0 values if no info about ipv4 pings' do
        expected_stats = { count: 0, avg: 0.0, max: 0.0, min: 0.0, stddev: 0.0, median: 0.0, timeout_percent: 0.0 }
